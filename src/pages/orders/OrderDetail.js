...
import { GET_ORDER, UPDATE_ORDER } from "../../graphql/orders";
...
const OrderDetail = ({ refreshOrders, orderNo }) => {
  ...
  const [getOrderDetailById] = useLazyQuery(GET_ORDER, {
    ...
    <>
      <div className="flex justify-between border-slate-100 bg-slate-50	p-4 my-4 rounded">
        <div>
          <div className="flex items-center">
            ...
            <span className="font-bold text-sky-600	text-base ml-1">#{orderDetail?.orderNo}</span>
            {/* <Button
              type='link'
              title="In bill"
              className="mt-[3px]"
          ...
          <div className="mt-5">
            ...
            <span className="text-base font-bold">{formatVNDMoney(orderDetail?.totalPrice + orderDetail?.totalDiscount - orderDetail?.shipmentFee )}đ</span>
          </div>
          <div className="mt-2">
            <span className="font-semibold">Tổng khuyễn mãi:</span> {' '}
            <span className="text-base font-bold">{formatVNDMoney(orderDetail?.totalDiscount)}đ</span>
          </div>
          <div className="mt-2">
            <span className="font-semibold">Phí ship:</span> {' '}
            <span className="text-base font-bold">{formatVNDMoney(orderDetail?.shipmentFee)}đ</span>
          </div>
          <div className="mt-2 mb-5">
            <span className="font-semibold">Tổng tiền:</span> {' '}
            <span className="text-base font-bold">{formatVNDMoney(orderDetail?.totalPrice)}đ</span>
          </div>
            ...
            orderDetail?.orderVouchers?.map(voucher => {
              return (
                <div key={voucher?.discountCode}>
                  <div className="mt-2">
                    <span className="font-semibold">Mã giảm giá: {voucher?.discountCode} - {voucher?.discountType} - {formatVNDMoney(voucher?.discountValue)} VND</span>
                  </div>
                </div>
              )
            })
          ...
          <div className="mt-2">
            ...
            <span className="text-base">{moment(orderDetail?.created_at).format(DateTimeFormat.TimeStamp)}</span>
          </div>
          <div className="mt-2">
            <span className="font-semibold">Thời gian mong muống nhận hàng:</span> {' '}
            <span className="text-base">{moment(orderDetail?.expectedDeliveryTime).subtract(7, 'hour').format(DateTimeFormat.TimeStamp)}</span>
          </div>
          <div className="mt-2">
        ...
        <div>
          <Tag
            color={EnumOrderColorStatuses[orderDetail?.status]}
          >
      ...
      <div className="flex justify-between my-4">
        <div className="border-2	border-slate-200	border-solid rounded-md w-3/12">
          <div className="font-bold mb-4 border-2	border-slate-200 bg-slate-100 p-4">KHÁCH HÀNG</div>
          <div className="p-4">
        ...
        <div className="border-2	border-slate-200	border-solid rounded-md w-8/12">
          <div className="font-bold mb-4 border-2	border-slate-200 bg-slate-100 p-4">NGƯỜI NHẬN</div>
          <div className="p-4">
            <div>{orderDetail?.orderAddress?.receiverPhone}</div>
            <div className="text-gray-400">{orderDetail?.orderAddress?.receiverPhone}</div>
            <div className="text-gray-400">
              {
                orderDetail?.orderAddress?.province
                  ?
                  <div>{''} {orderDetail?.orderAddress?.street + ", " + orderDetail?.orderAddress?.ward + ", " + orderDetail?.orderAddress?.district + ", " + orderDetail?.orderAddress?.province}  </div>
                  :
          ...
          orderStatusBtn[orderDetail?.status]?.map(status => {
            return <>
              <button
                ...
                onClick={() => { updateStatusOrder(status) }}
              >
                <span className="font-bold text-white">{EnumOrderStatuses[status]}</span>
              </button>
            </>
          })
        }
