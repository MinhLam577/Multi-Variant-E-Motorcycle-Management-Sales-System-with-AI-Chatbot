...
const getColumnsConfig = ({ handleViewProductsCombo }) => {
  return [
    {
      title: "Tên món ăn",
      dataIndex: "comboName",
      ...
      render: (value, item) => {
        return (
          <Button
            ...
            onClick={() =>
              typeof handleViewProductsCombo === "function" &&
              handleViewProductsCombo(item)
            }
          >
            {value}
          </Button>
  ...
  const getColumnsConfigProducts = () => {
    return [
      ...
      },
      {
        ...
        render: (value, item) => (
          <div>
            {formatVNDMoney(item.price)}/{item.oneUnit} {item.unit}
          </div>
        ),
      },
      {
