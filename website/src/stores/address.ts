...
export default class AddressObservable {
    ...
    *getAddressDefault(idCustomer: string) {
        try {
            ...
            const response: ResponsePromise =
                yield AddressApi.getAddressDefault(idCustomer);
    ...
    *getListAddress(idCustomer: string) {
        try {
            this.loading = true;
            const response = yield AddressApi.getListAddressByCustomer(
                idCustomer
            );
            const { data, status, message } = response;
            const success_status = [200, 201, 204];
    ...
    *postAddress(formdata) {
    ...
    //Tạo địa chỉ , tạo kèm theo mặc định nếu là 1 phần tử ở checkout

    *postAddress_Checkout(formdata) {
        try {
            this.loading = true;
            const response = yield AddressApi.createAddress(formdata);
            const { data, status, message } = response;
            ...
            if (success_status.includes(status)) {
                ...
                if (this.data.listAddress.length == 1) {
                    yield this.updateAddressDefaultCustomer(
                        formdata.customerId,
                        data.id
                    );
                }
