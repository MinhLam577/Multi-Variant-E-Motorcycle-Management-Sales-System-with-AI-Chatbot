...
export interface OrderAmountDetailsProps {
    listDataSelected: CartItemResponseType[];
}
const OrderAmountDetails: React.FC<OrderAmountDetailsProps> = observer(
    ({ listDataSelected }) => {
        ...
        return (
            <ul className="divide-y divide-gray-200 text-sm text-gray-700">
                            ...
                            <span className="text-orange-600">
                                ...
                                {storeVoucher?.dataDetail?.voucher?.fixed
                                    ? calculatePercentDiscount(
                                          storeVoucher?.dataDetail?.voucher
                                              ?.discount_amount || 0,
                                          subtotal
                                      )
                                    : formatCurrency(
                ...
                <li className="font-bold text-lg border-t border-dashed mt-2 pt-4">
                    <p className="flex justify-between">
                        <span>Tổng cộng</span>
                        <span className="text-orange-600">
