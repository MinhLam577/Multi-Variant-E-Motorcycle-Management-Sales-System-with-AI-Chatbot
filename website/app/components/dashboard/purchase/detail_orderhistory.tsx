...
export default function DetailOrderHistory({ slug }) {
  ...
  useEffect(() => {
    const getDetailOrder = async () => {
      ...
      console.log(OrderStore.data.order_detail);
            ...
            <div className="flex p-2 mt-4" key={item.id}>
              ...
              <div className="ml-3 flex-grow">
                <div className="flex flex-col justify-between gap-y-1">
                  ...
                  >
                    <p className="font-semibold">{item.skus.product.title}</p>
                  </Link>
              ...
              <div className="flex ml-3 items-center gap-4">
                <span className="line-through text-gray-500">
                  ₫{parseFloat(item.skus.price_sold).toLocaleString()}
                </span>
                <span className="text-red-500 font-semibold">
                  ₫{(item.skus.price_sold * item.quantity).toLocaleString()}
                </span>
              </div>
            </div>
          );
