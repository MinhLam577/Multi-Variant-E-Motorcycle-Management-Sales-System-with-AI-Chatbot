    ...
    const fetchBlogDetail = async () => {
        try {
            ...
            if (dataAccount) {
                const arrThumbnail = [
                    {
                        ...
                        name: "Avatar",
                        status: "done",
                        url: dataAccount.user.avatarUrl,
                        thumbnail: dataAccount.user.avatarUrl,
                    },
                ];
                const init = {
                    id: dataAccount?.user.userId,
                    username: dataAccount?.user.username,
                    email: dataAccount?.user.email,
                    phoneNumber: dataAccount?.user.phoneNumber,
                    gender: dataAccount?.user.gender,
                ...
                form.setFieldsValue({
                    ...init,
                    birthday: dataAccount?.user.birthday
                        ? dayjs(dataAccount.user.birthday)
                        : null,
                });
    ...
    const handleFormFinish = async (values) => {
        ...
        try {
            const data = await apiClient.patch(
                endpoints.customers.update(storeAccount.account?.user.userId),
                updateInfo
            );
            if (data.status == 200) {
                const dataAccount = {
                        ...
                        <Col span={12}>
                            <Form.Item label="Giới tính" name="gender">
                                <Select
                                    ...
                                    options={Object.keys(GenderType).map(
                                        (item) => ({
                                            value: item,
                                            label: UserType[item],
                                        })
